...
if(silence.jaja === true && (is_url(message.content)) && message.member?.permissions.has("ADMINISTRATOR")) {
  try {
  	message.delete().then((m: any) => m.channel.send({ embeds: [embed]}))
} catch (error) {
  console.log(error)
...
/*    const dsnte = new Discord.MessageEmbed()
  ...
  message.reply({ embeds: [dsnte] })  */
	return message.reply(`No tengo un comando llamado \`${command}\` pero puedes usar \`${p}help\``);
}
}
}
