...
import {
  myMention,
  randomsPacmansReplies,
  randomizePacmanReplies,
  pacmansReplies,
} from "../resources";
  ...
  run: async (client, message) => {
    ...
    if (message.inGuild()) {
      ...
      let pacmansConfig =
        ...
        (await new la_grasa(pacmansParams));
      ...
      if (pacmansConfig.sdlg == true && message.content === ":v") {
        message.channel.send(
          pacmansReplies[~~(Math.random() * pacmansReplies.length)]
        );
      }
      ...
      if (
        ...
        try {
          const noLinksEmbed = new Discord.MessageEmbed()
            ...
            .setDescription(
              "No puedes enviar links, el antilink está activado."
            )
            .setFooter(
          ...
          message.delete().then((m: Message) =>
            m.channel.send({ embeds: [noLinksEmbed] }).then((m) => {
              setTimeout(() => {
                m.delete();
              }, 10000);
            })
    ...
    const latPerms = {
      ...
      PRIORITY_SPEAKER:
        "Tener prioridad de palabra (Volumen más alto en comparación a los otros miembros en un canal de voz)",
      STREAM: "Transmitir",
      ...
      READ_MESSAGE_HISTORY:
        "Leer el historial de mensajes en un canal de texto",
      ...
      DEAFEN_MEMBERS:
        "Remover el audio de los miembros en los canales de voz (no escuchar audio en un canal de voz)",
      ...
      MANAGE_NICKNAMES:
        "Cambiar o reestablecer los apodos (nicknames) de los miembros",
      MANAGE_ROLES:
        "Gestionar los roles del servidor (eliminarlos o cambiar su información)",
      MANAGE_WEBHOOKS:
        "Gestionar las webhooks del servidor, eliminarlas o crear nuevas",
      MANAGE_EMOJIS_AND_STICKERS:
        "Gestionar los emojis y/o stickers del servidor (eliminarlos, crear nuevos o cambiar información de los ya existentes)",
      USE_APPLICATION_COMMANDS:
        "Utilizar Interacciones de los bots (Slash Commands y Context Menus)",
      REQUEST_TO_SPEAK:
        "Permiso para solicitar hablar en un canal de escenario",
      MANAGE_EVENTS:
        "Gestionar los eventos programados de un servidor (eliminarlos, crear nuevos o cambiar información de los ya existentes)",
      MANAGE_THREADS:
        "Gestionar los hilos de los canales (eliminarlos, crear nuevos o cambiar información de los ya existentes)",
      ...
      START_EMBEDDED_ACTIVITIES:
        "Iniciar una actividad en un canal de voz (exclusivo para bots)",
    ...
    if (cmd?.guildOnly && !message.inGuild()) {
      return message.reply(
        "Este comando solo está habilitado para los servidores."
      );
    }
