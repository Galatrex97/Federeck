...
const pagination = async (options) => {
    ...
    const changeFooter = () => {
        ...
        if (embed?.footer?.text) {
            return newEmbed.setFooter(`${embed.footer.text} - Página ${currentPage} de ${embeds.length} - reconlx npm`, embed.footer.iconURL);
        }
        return newEmbed.setFooter(`Página ${currentPage} de ${embeds.length} - reconlx npm`);
    };
    ...
    const numberTravel = async () => {
        if (pageTravelling.has(author.id))
            return channel.send("Escribe \".\" para ir a la página");
        const collector = channel.createMessageCollector({
            filter: (msg) => msg.author.id === author.id,
            time: 15000,
        });
        const numberTravelMessage = await channel.send(`Escribe un número ${author}, tienes 15 segundos.`);
