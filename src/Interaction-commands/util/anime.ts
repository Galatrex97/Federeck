import { Client, CommandInteraction, MessageEmbed, ChannelResolvable, TextChannelResolvable, TextChannel, GuildChannel } from "discord.js";
...
let Text = interaction.options.getString("search");
  ...
  if(Text && Text.length > 200) return interaction.followUp({content: `<a:noputo:868687565304246283> | **El limite de texto es de 200**`, ephemeral: true}); //limite de texto
...
if(Text) {
  ...
  } catch (error: any) { //abrimos y cerramos el evento
    console.log(error)
...
let channel = client.channels.cache.get("901550991923630130") as TextChannel;
