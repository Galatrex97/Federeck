...
import { CategoryChannel, TextChannel, Message, MessageEmbed } from "discord.js";
...
export const command: Command = {
  ...
  usage: 'new',
	run: async (client: Klar, message: Message, args: String[0], p: string) => {
     ...
     if(!data.parentId) return message.channel.send(`No has establecido una categoría para los tickets. Usa ${p}ticket-setup.`)

let category = message.guild?.channels.resolve(data.parentId);

if((category as CategoryChannel).type !== "GUILD_CATEGORY") return message.reply("Necesitas volver a configurar los tickets ya que la categoria ha dejado de existir o no es válida, usa "+p+"setup para configurar los tickets e nuevo.")
...
if(!message.guild?.me?.permissions.has("MANAGE_CHANNELS")) return message.reply("No puedo. intenta ponerme un rol con el permiso **Gestionar Canales**")
      ...
      parent: data.parentId,
			...
			if (logchannel) (logchannel as TextChannel).send(`Ticket de ${message.author} creado. <#${channel.id}>`);
