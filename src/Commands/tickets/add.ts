...
export const command: Command = {
	...
	run: async (client: Klar, message: Message, args: String[], p: string) => {
		...
		if((message.channel as TextChannel).name.includes('ticket-')) {
			const member = message.mentions.members?.first() || message.guild?.members.cache.get(args[0] as string) || message.guild?.members.cache.find(x => x.user.username === args.slice(0).join(' ') || x.user.username === args[0]);
			if(!member) {
