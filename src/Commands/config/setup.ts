...
export const command: Command = {
  ...
  usage: 'setup',
	run: async (client: Klar, message: Message, args: string, p: string) => {
        ...
        if(!idParent) return;
     let data = await nyaSchema.findOne({ guildId : message.guild?.id, parentId: idParent})
     ...
     if(!idParent) return message.channel.send(`Necesitas dar el id de una categoría.`)

     let category: any = message.guild?.channels.resolve(idParent)
     
     if(category.type !== "GUILD_CATEGORY") return message.reply("Esa no es una categoria válida")
