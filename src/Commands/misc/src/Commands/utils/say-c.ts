import Discord, { Channel, Client, MessageEmbed, Role, GuildMember, Message, TextChannel } from "discord.js";
...
if (texto.includes("@everyone") && !perms) return message.reply("Para mencionar debes tener el permiso de Administrador.").then(e =>{
  setTimeout(() =>{
    e.delete()
  }, 7000)
})
...
if(!(message.member as unknown as GuildMember).permissions.has("ADMINISTRATOR") && (is_url(texto) === true)) return message.reply("Para mencionar debes tener el permiso de Administrador.")


if(texto.includes(roleMention as any) && !message.member?.permissions.has("ADMINISTRATOR")) {
  return message.reply("Para mencionar debes tener el permiso de Administrador.")
}
if (texto.includes("@here") && !perms) return message.reply("Para mencionar debes tener el permiso de Administrador.").then(e =>{
  setTimeout(() =>{
    e.delete()
  }, 7000)
})
