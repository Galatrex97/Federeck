import Discord, { Client, Message, Guild, MessageEmbed, GuildMember } from "discord.js";
...
if(user.id === owner) return message.reply("No se puede kickear al dueño del servidor. XD?")
  ...
  if((message.member as GuildMember).roles.highest.comparePositionTo(user.roles.highest) <= 0) return message.reply("No puedes banear a esa persona por que tiene un mayor o igual rango que tú.")
